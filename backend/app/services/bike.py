"""Business logic for Honda 125 bike system."""
...
VALID_UPGRADE_COMPONENTS = [
    "engine", "turbo", "exhaust", "brake", "wheel", "tire",
    "suspension", "fuel_tank", "seat", "headlight", "tail_light",
    "indicator", "handlebar", "horn", "chain", "mirror", "speedometer",
]

INSURANCE_TIERS = {
    "basic": {"monthly_premium": 50, "coverage": 5000, "deductible": 500},
    "standard": {"monthly_premium": 150, "coverage": 20000, "deductible": 200},
    "premium": {"monthly_premium": 500, "coverage": 100000, "deductible": 0},
}

REPAIR_COST_PER_UNIT = {
    "engine": 25, "body": 10, "brakes": 15, "suspension": 20,
    "wheels": 12, "tires": 8, "full": 50,
}
...
class BikeService:
    def __init__(self, session: AsyncSession):
        ...
        self.bike_repo = BikeRepository(session)
    ...
    async def get_variants(self, skip: int = 0, limit: int = 50) -> list:
        return await self.bike_repo.get_all_variants(skip=skip, limit=limit)

    async def get_variant(self, variant_id: UUID):
        variant = await self.bike_repo.get_variant_by_id(variant_id)
        if not variant:
            raise NotFoundError("Bike variant not found")
        return variant

    async def purchase_bike(self, player_id: UUID, variant_id: UUID, bike_name: str) -> dict:
        variant = await self.bike_repo.get_variant_by_id(variant_id)
        if not variant:
            raise NotFoundError("Bike variant not found")
    ...
    async def sell_bike(self, player_id: UUID, vehicle_id: UUID) -> dict:
        ...
        bike = await self.bike_repo.get_by_vehicle_id(vehicle_id)
        if not bike:
            raise NotFoundError("Bike record not found")

        variant = await self.bike_repo.get_variant_by_id(bike.variant_id)
        if not variant:
            raise NotFoundError("Bike variant not found")
        ...
        await self.bike_repo.delete(bike.id)
        await self.vehicle_repo.soft_delete(vehicle_id)

        return {
            "sold_price": sold_price,
            "message": f"Bike '{vehicle.name}' sold for ${sold_price}",
        }

    async def get_player_bikes(self, player_id: UUID) -> list[dict]:
        bikes = await self.bike_repo.get_by_player_id(player_id)
        result = []
        for b in bikes:
            variant = await self.bike_repo.get_variant_by_id(b.variant_id)
            vehicle = await self.vehicle_repo.get_by_id(b.vehicle_id)
            result.append({
                "vehicle": vehicle,
                "bike": b,
                "variant": variant,
    ...
    async def get_bike_detail(self, player_id: UUID, vehicle_id: UUID) -> dict:
        vehicle = await self.vehicle_repo.get_by_id(vehicle_id)
        if not vehicle or vehicle.player_id != player_id:
            raise NotFoundError("Bike not found")
        ...
        variant = await self.bike_repo.get_variant_by_id(bike.variant_id)
        insurance = await self.bike_repo.get_insurance(vehicle_id)
    ...
    async def upgrade_component(
        ...
        current_level = getattr(bike, level_field, 1)
        ...
        await self.bike_repo.update(bike.id, {level_field: target_level})

        updated_bike = await self.bike_repo.get_by_vehicle_id(vehicle_id)
        effective_stats = self._compute_effective_stats(variant, updated_bike)
    ...
    async def update_customization(
        ...
        allowed_keys = {"paint_id", "decal_id", "sticker_id", "license_plate_style"}
        ...
        await self.bike_repo.update(bike.id, update_data)
        return await self.bike_repo.get_by_vehicle_id(vehicle_id)
    ...
    async def get_performance_stats(self, vehicle_id: UUID) -> dict:
        ...
        return self._compute_effective_stats(variant, bike)
    ...
    async def purchase_fuel(
        ...
        station = await self.bike_repo.get_fuel_station(station_id)
        if not station:
        ...
        await self.bike_repo.add_fuel_transaction({
    ...
    async def repair_bike(
        ...
        await self.bike_repo.add_repair_record({
            "vehicle_id": vehicle_id,
            "garage_id": garage_id,
            "repair_type": repair_type,
    ...
    async def purchase_insurance(
        ...
        existing = await self.bike_repo.get_insurance(vehicle_id)
        if existing and existing.is_active:
            raise ConflictError("Active insurance policy already exists for this bike")

        tier_info = INSURANCE_TIERS[tier]
        await self._charge_wallet(player_id, tier_info["monthly_premium"])

        insurance = await self.bike_repo.create_insurance({
            "vehicle_id": vehicle_id,
            "tier": tier,
    ...
    async def get_insurance(self, vehicle_id: UUID) -> BikeInsurance | None:
        insurance = await self.bike_repo.get_insurance(vehicle_id)
        if insurance and insurance.expires_at:
            if insurance.expires_at < datetime.now(timezone.utc):
                await self.bike_repo.update(insurance.id, {"is_active": False})
                return None
        return insurance

    def _compute_effective_stats(self, variant, bike) -> dict:
        upgrade_levels = []
        for field in UPGRADE_FIELDS:
            level_field = f"{field}_level"
            level = getattr(bike, level_field, 1)
